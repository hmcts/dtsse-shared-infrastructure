moved {
  from = azurerm_dashboard_grafana.dashboard-grafana
  to   = azurerm_dashboard_grafana.main_west
}

moved {
  from = azurerm_dashboard_grafana.dashboard-grafana10
  to   = azurerm_dashboard_grafana.main
}

resource "azurerm_dashboard_grafana" "main_west" {
  count                             = var.dashboard_count
  name                              = "${var.product}-grafana-${var.env}-west"
...
resource "azurerm_role_assignment" "viewers-grafana9" {
  count                = var.dashboard_count
  scope                = azurerm_dashboard_grafana.main_west[0].id
  role_definition_name = "Grafana Viewer"
  principal_id         = data.azuread_group.viewers.object_id
}
...
resource "azurerm_role_assignment" "app_insights_dcd_cnp_prod_access-grafana9" {
  ...
  scope                = "/subscriptions/8999dec3-0104-4a27-94ee-6588559729d1"
  role_definition_name = "Reader"
  principal_id         = azurerm_dashboard_grafana.main_west[0].identity[0].principal_id
}
